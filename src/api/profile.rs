...
pub async fn update_profile(
    ...
    if let Some(existing_profile) = profile {
        ...
        if let Some(avatar_config) = req.avatar_config {
             active.avatar_config = Set(Some(serde_json::to_string(&avatar_config).unwrap_or_default()));
        }
        ...
        use crate::models::library_config::{ActiveModel as ConfigActiveModel, Entity as ConfigEntity};
