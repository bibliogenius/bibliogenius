...
pub(crate) async fn sync_all_peer_scores(state: &AppState) {
    ...
    tracing::info!(
        "hangman sync phase 1 done in {}ms: direct_ok={}, direct_fail={}, relay_queued={}",
    ...
    if !relay_peers.is_empty() {
        ...
        for (peer, bundle) in relay_results {
            let Some(bundle) = bundle else {
                relay_no_response += 1;
                tracing::info!(
                    "hangman sync: relay got no response from peer '{}' (id={})",
                    peer.name,
                    peer.id,
                );
                continue;
        ...
        tracing::info!(
            "hangman sync phase 2 done in {}ms: relay_sent={}, relay_ok={}, relay_no_response={}",
        ...
        tracing::info!("hangman sync: no relay peers queued, skipping phase 2");
    }

    tracing::info!(
        "hangman sync completed in {}ms total",
        sync_start.elapsed().as_millis(),
    );
}
